...
export function queryController({ queries, logger }) {
  return {
    getSessionOrders: asyncHandler(async (req, res) => {
      const { sessionId } = req.params;
      const { cursor, limit } = req.query;

      const result = await queries.sessionOrders.getBySession(sessionId, {
        cursor,
        limit: limit ? Number(limit) : undefined,
      });
    ...
    getOrderDetail: asyncHandler(async (req, res) => {
      ...
      const order = await queries.orderDetail.getById(orderId);
    ...
    getReceptionQueue: asyncHandler(async (req, res) => {
      ...
      const result = await queries.queues.getQueue("queue:reception", {
        offset: offset ? Number(offset) : 0,
        limit: limit ? Number(limit) : undefined,
      });
    ...
    getKitchenQueue: asyncHandler(async (req, res) => {
      ...
      const result = await queries.queues.getQueue("queue:kitchen", {
        offset: offset ? Number(offset) : 0,
        limit: limit ? Number(limit) : undefined,
      });
    ...
    getCompletedOrders: asyncHandler(async (req, res) => {
      ...
      const result = await queries.completedOrders.getCompleted({
        offset: offset ? Number(offset) : 0,
        limit: limit ? Number(limit) : undefined,
      });
